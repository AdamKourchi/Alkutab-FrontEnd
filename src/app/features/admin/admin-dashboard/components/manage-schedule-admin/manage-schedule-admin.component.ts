...
@Component({
  selector: 'app-manage-schedule-admin',
  imports: [
    MatCardModule,
    FullCalendarModule,
    CommonModule,
  ...
  templateUrl: './manage-schedule-admin.component.html',
  styleUrl: './manage-schedule-admin.component.css',
})
export class ManageScheduleAdminComponent {
  ...
  createCalendarEvents(paths: Path[], sessions: any[]) {
    ...
    this.allEvents = [...this.allLevelEvents, ...this.allSessionEvents];
  ...
  handleEventReceive(eventInfo: any) {
    ...
    // Set a default duration of 1 hour if the event has no end date
    if (!event.end) {
